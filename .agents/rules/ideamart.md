...
Ideamart also runs **OmniAI**, an AI gateway: one OpenAI-shaped API in front of Claude, Gemini
and GPT models, plus image generation, billed from an Ideamart token balance. It shares the
brand and the host and almost nothing else, so it has its own section at the end of this file
and its own reference, [references/14-omni-ai.md](references/14-omni-ai.md). **Do not apply the
telco rules below to an OmniAI call, or the OmniAI rules to a telco one.**
...
direct carrier billing, telco SMS in Sri Lanka, or OmniAI / `omniai.ideamart.io`.**
...
6. **Never let subscriber identity reach an OmniAI prompt**, and cap every AI call with
...
is not installed — read `catalog/ideamart-api.json` directly; it is plain JSON with the same
...
are identical everywhere, so no language is second-class. `node tools/ideamart.mjs curl <id>
...
someone wrote emitters for and ages with those languages' idioms rather than with the Ideamart
...
| **OmniAI** — the AI gateway: auth, models, chat completions, image generation, errors | [references/14-omni-ai.md](references/14-omni-ai.md) |
...
the curl reference — and never introduce a new runtime to reach Ideamart.

---

## Service map

Production host `https://api.ideamart.io`; LBS is on `https://api.dialog.lk/lbs/locate`.
...
| **Ask a model / generate text** | `POST https://api.ideamart.io/omniai/api/v1/chat/completions` |
| **Generate an image** | `POST https://api.ideamart.io/omniai/api/v1/images/generations` |
...
- ❌ Checking the HTTP status as success (`res.ok`, `raise_for_status()`,
  `EnsureSuccessStatusCode()`, `http_errors`) — **Ideamart returns 200 for errors.** Branch on
  `statusCode`.
...
- ❌ `NEXT_PUBLIC_` / `VITE_` / `REACT_APP_` / `PUBLIC_` / `EXPO_PUBLIC_` on any Ideamart
  variable, or serving it through a client-facing config endpoint — that ships the password to
  the browser.
- ❌ Standing up a Node sidecar (or any second runtime) to call Ideamart from a non-JS project.
- ❌ Logging `password`, the OTP, `referenceNo`, or an unmasked `subscriberId`.
- ❌ Inventing IVR endpoints — there is no public specification.
- ❌ Sending an OmniAI key as `Bearer …`, or sending it in the JSON body — it goes in the
  `Authorization` header **verbatim**, `app_<keyId>.<keyValue>`.
- ❌ Looking for `statusCode` in an OmniAI response, or reusing the telco client for it — OmniAI
...
## OmniAI — the AI gateway

`https://api.ideamart.io/omniai/api` · portal <https://omniai.ideamart.io> · full contract
[references/14-omni-ai.md](references/14-omni-ai.md) · `node tools/ideamart.mjs omniai`
...
bearer token: no `Bearer` prefix, no base64. From `OMNIAI_API_KEY`, backend only. It spends a
...
**When you generate OmniAI code, always:**
...
- Treat model output as untrusted input: never execute it, never interpolate it into SQL or a
  ...
  Sinhala or Tamil — every part is billed).
