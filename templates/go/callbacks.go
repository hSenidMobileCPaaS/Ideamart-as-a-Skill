...
// Ideamart callback (inbound webhook) handlers — net/http.
//
...
func (c *Callbacks) Register(mux *http.ServeMux) {
	mux.HandleFunc("/api/ideamart/sms/mo", c.MoSMS)
	mux.HandleFunc("/api/ideamart/sms/dlr", c.DeliveryReport)
	mux.HandleFunc("/api/ideamart/ussd", c.USSD)
	mux.HandleFunc("/api/ideamart/subscription/notification", c.SubscriptionNotification)
	mux.HandleFunc("/api/ideamart/charging/notification", c.ChargingNotification)
}

/* ── Shared guards ───────────────────────────────────────────────────────── */

// IdeamartSourceIPs restricts callbacks to Ideamart's egress addresses. Ideamart
// signs nothing, so there is no signature to verify — source IP is the strongest
// control available. Ask support for the current list. Prefer enforcing it at
// the firewall or load balancer; this is the fallback for when you cannot.
var IdeamartSourceIPs []string

func allowedSource(r *http.Request) bool {
	if len(IdeamartSourceIPs) == 0 {
		return true // not configured yet
	}
	ip := strings.TrimSpace(strings.Split(r.Header.Get("X-Forwarded-For"), ",")[0])
	for _, allowed := range IdeamartSourceIPs {
		if ip == allowed {
			return true
		}
	}
...
// isOurApp rejects payloads addressed to a different application. Cheap noise filter.
...
func (c *Callbacks) enqueue(name string, payload map[string]any) {
	...
	default:
		log.Printf("[ideamart] job queue full, dropping %s", name)
	}
}
...
func (c *Callbacks) MoSMS(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[ideamart] mo-sms requestId=%s from=%s",
...
// deliveryStatus normalises the two spellings Ideamart and the SMPP gateway use.
...
func (c *Callbacks) DeliveryReport(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[ideamart] delivery-report requestId=%s status=%s",
...
// USSD acknowledges only. The screen the user sees comes from a separate
...
func (c *Callbacks) USSD(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[ideamart] ussd sessionId=%s operation=%s from=%s",
...
func (c *Callbacks) handleUSSDInput(payload map[string]any) {
	...
	send := func(message, operation string) {
		if _, err := c.Client.SendUSSD(ctx, sessionID, source, message, operation); err != nil {
			log.Printf("[ideamart] ussd send failed sessionId=%s: %v", sessionID, err)
		}
	}
...
// including changes you did not initiate (a user texting STOP, an operator
...
func (c *Callbacks) SubscriptionNotification(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[ideamart] subscription-notification subscriber=%s status=%s",
...
// ChargingNotification is your reconciliation channel. Every charge left unknown
...
func (c *Callbacks) ChargingNotification(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[ideamart] charging-notification externalTrxId=%s statusCode=%s",
...
func (c *Callbacks) worker() {
	for next := range c.Jobs {
		...
		default:
			log.Printf("[ideamart] unknown job %s", next.name)
		}
	}
}
...
// SessionStore keeps USSD menu position, keyed by sessionId with a ~2 minute TTL.
//
// It MUST be shared across instances in production: a keypress routed to another
// instance cannot see an in-process map, and the user's menu dies mid-flow. Back
// it with Redis and this interface stays the same.
...
/* ── Wiring ───────────────────────────────────────────────────────────────
 *
 *   config, err := ideamart.LoadConfig()
 *   if err != nil { log.Fatal(err) }            // fail at boot, not under load
 *
 *   client := ideamart.NewClient(config)
 *   callbacks := ideamart.NewCallbacks(config, client, ideamart.NewMemorySessionStore())
 *
