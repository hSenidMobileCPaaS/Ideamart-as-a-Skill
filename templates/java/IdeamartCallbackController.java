...
/**
 * Ideamart callback (inbound webhook) handlers — Spring Boot.
 *
 * <p>Routes to register in the IdeaPro portal:
 *
 * <pre>
 ...
 */
@RestController
@RequestMapping("/api/ideamart")
public class IdeamartCallbackController {

  private static final Logger log = LoggerFactory.getLogger(IdeamartCallbackController.class);
  ...
  private final IdeamartClient client;
  private final UssdSessionStore sessions;
  private final DedupeStore dedupe = new DedupeStore(Duration.ofMinutes(10));

  public IdeamartCallbackController(
      IdeamartConfig config, IdeamartClient client, UssdSessionStore sessions) {
    this.config = config;
    this.client = client;
  ...
  public ResponseEntity<Map<String, String>> moSms(@RequestBody Map<String, Object> body) {
    ...
    log.info(
        ...
        body.get("requestId"),
        IdeamartClient.maskAddress(String.valueOf(body.get("sourceAddress"))));
  ...
  public ResponseEntity<Map<String, String>> subscriptionNotification(
    ...
    log.info(
        ...
        IdeamartClient.maskAddress(String.valueOf(body.get("subscriberId"))),
        body.get("status"));
  ...
  /**
   * Your reconciliation channel. Every charge left UNKNOWN after a timeout gets resolved here.
  ...
  void handleAsync(String job, Map<String, Object> payload) {
    ...
    } catch (IdeamartClient.IdeamartException e) {
