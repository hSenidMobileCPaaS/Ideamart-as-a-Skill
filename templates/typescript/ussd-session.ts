 ...
 * USSD session store.
 *
 * A USSD session is short-lived and stateful. The platform sends you a
 * keypress and a `sessionId`; it does not tell you where in the menu the user
 * is. That state is yours to keep.
 *
 ...
 * instance B cannot see a session created on instance A, and the user's menu
 * dies mid-flow. It also loses every live session on deploy. Use the Redis
 * implementation at the bottom of this file in production.
 */

export interface UssdSession {
  /** Where in your menu tree the user currently is. */
  node: string;
 ...
 * Menu tree
 *
 * A declarative tree keeps screens under the ~160-character limit and makes
 * the mt-cont / mt-fin decision structural rather than something you remember
 * to get right at each branch.
 * ───────────────────────────────────────────────────────────────────────── */
...
/**
 * USSD is plain ASCII (encoding 440) and roughly 182 characters per screen.
 * Strip anything else and truncate at a safe 160 — a screen that silently
 * fails to render tells the user nothing.
 */
