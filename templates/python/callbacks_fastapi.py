"""
Ideamart callback (inbound webhook) handlers — FastAPI.
...
from ideamart_client import mask_address, send_ussd, IdeamartError
from ideamart_config import config
from ussd_session import end_session, get_session, set_session

logger = logging.getLogger("ideamart")
router = APIRouter(prefix="/api/ideamart")
...
def parse_ideamart_timestamp(raw: str) -> Optional[datetime]:
    """
...
async def charging_notification(
    ...
    resolved here. Idempotency is not optional — a duplicate that double-counts
    revenue is a real bug with real consequences.
    ...
    else, and log the raw body on your first Limited Production charge so you
    can widen the handler if your account sends extra fields.
    """
    if not is_allowed_source(request):
        return ack()
...
# middleware — then rely on the Ideamart source-IP allowlist instead, or you
