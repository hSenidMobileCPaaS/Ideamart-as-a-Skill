"""
Ideamart configuration — the ONLY module that reads the environment.
...
def _require_env(name: str) -> str:
    ...
    if not value:
        raise RuntimeError(
            f"[ideamart] Missing required environment variable {name}.\n"
            f"Copy .env.example to .env and fill in your Ideamart credentials.\n"
...
_ENDPOINT_VARS: Dict[str, str] = {
    "sms_send": "IDEAMART_SMS_SEND_URL",
    "ussd_send": "IDEAMART_USSD_SEND_URL",
    "subscription_send": "IDEAMART_SUBSCRIPTION_SEND_URL",
    "subscription_status": "IDEAMART_SUBSCRIPTION_STATUS_URL",
    "subscription_query_base": "IDEAMART_SUBSCRIPTION_QUERY_BASE_URL",
    "otp_request": "IDEAMART_OTP_REQUEST_URL",
    "otp_verify": "IDEAMART_OTP_VERIFY_URL",
    "caas_debit": "IDEAMART_CAAS_DEBIT_URL",
    "caas_balance": "IDEAMART_CAAS_BALANCE_URL",
    "lbs_locate": "IDEAMART_LBS_URL",
}


@dataclass(frozen=True)
class IdeamartConfig:
    ...
    def require_endpoint(self, service: str) -> str:
        ...
        if service not in _ENDPOINT_VARS:
            raise KeyError(f"[ideamart] Unknown service '{service}'")
        url = self.endpoints.get(service)
        if not url:
            raise RuntimeError(
                f"[ideamart] {service} is not configured. Either the API is not enabled "
                f"on your application in IdeaPro, or {_ENDPOINT_VARS[service]} is missing "
...
def load_config() -> IdeamartConfig:
    return IdeamartConfig(
