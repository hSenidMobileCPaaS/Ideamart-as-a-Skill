...
A USSD session is short-lived and stateful. The platform sends you a keypress
and a `sessionId`; it does not tell you where in the menu the user is. That
...
B cannot see a session created on worker A, and the user's menu dies mid-flow.
Gunicorn/uvicorn with more than one worker is already "more than one process".
Use the Redis implementation at the bottom of this file in production.
"""
...
# Strip anything else and truncate at a safe 160 — a screen that silently fails
# to render tells the user nothing.
# ─────────────────────────────────────────────────────────────────────────────
