...
the project's HTTP client and you have the call, whatever the language. Reference
implementations and code emitters exist for TypeScript/Node, Python, Java, Go, PHP and C# as a
convenience on top of that, and [references/11-any-stack.md](references/11-any-stack.md)
specifies the surrounding integration language-neutrally.
...
contract → the curl reference or `codegen` for the call → `validate` the payload → `code` /
`diagnose` when something fails.

### Write the call from the contract, not from memory

**Any language — [references/13-curl-reference.md](references/13-curl-reference.md).** Every
endpoint at the wire: a runnable curl, every parameter defined with type and requiredness, the
exact response, every response field explained, the status codes that endpoint returns, and the
same treatment for all five inbound callbacks including a command that replays each one against
your handler. Porting is mechanical — body, headers and branching are identical everywhere.
Run the curl by hand when a call fails; it separates a bad payload from bad code in one step.

**The six emitter languages — `codegen`.** Working code from the same contract, so the payload,
benign codes, timeout and `statusCode` branching cannot drift. Adapt it to the project's
conventions.
...
No emitter for your stack? Work from the curl reference — never introduce one of these six
languages into a project that is not written in it.
...
different structure — and never introduce a new runtime to reach Ideamart.
