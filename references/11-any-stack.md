...
Ideamart is JSON over HTTPS with a shared-secret credential pair. **Nothing about it requires
...
```
IDEAMART_APP_ID          required — fail to start without it
IDEAMART_PASSWORD        required — fail to start without it
IDEAMART_<SERVICE>_URL   optional — absent means "not provisioned, refuse to call"
```
...
```
input                        output
tel:94771234567          →   tel:94771234567     (already prefixed — return unchanged)
tel:hu3b84346f…          →   tel:hu3b84346f…     (masked hash — opaque, never parse)
+94 77 123 4567          →   tel:94771234567
0094771234567            →   tel:94771234567
077 123 4567             →   tel:94771234567     (local form → country code)
""                       →   error
```
...
function post(service, url, body, benignCodes = []):
    ...
    raise IdeamartError(data.statusCode, data.statusDetail, service, data)
```

Three things this pseudocode says that are easy to get wrong in a port:

- **The HTTP status is never consulted.** Ideamart returns 200 for application-level failures.
...
machine-readable data, in [`catalog/ideamart-api.json`](../catalog/ideamart-api.json) under
...
**Client-side prefixes to never use on an Ideamart variable**, since every framework has one:
...
- **Serverless functions** (Lambda, Cloud Run, Vercel, Azure Functions) have rotating egress
  IPs, and Ideamart enforces IP whitelisting (`E1303`). Route through a NAT gateway with a
...
- **Edge runtimes** (Cloudflare Workers, Deno Deploy, Vercel Edge) additionally give you no
  control over egress IP at all. Do not put Ideamart calls there.
- **Mobile and desktop clients** never call Ideamart directly, in any language. They call your
  backend.
...
1. Exactly one module reads the environment, and the process refuses to start with
   `IDEAMART_APP_ID` or `IDEAMART_PASSWORD` missing.
...
```bash
jq '.services[] | select(.id=="caas-direct-debit")' catalog/ideamart-api.json
python -c "import json;print(json.load(open('catalog/ideamart-api.json'))['statusCodes']['E1303'])"
```
