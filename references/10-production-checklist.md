...
- [ ] No `NEXT_PUBLIC_` / `VITE_` / `REACT_APP_` prefix on any Ideamart variable
...
- [ ] Production egress IP confirmed with `curl -4 https://myip.ideamart.io` **on the
      production server**
...
- [ ] Callback endpoints restricted to Ideamart source IPs
...
- [ ] Someone owns the Ideamart account and can log in to rotate credentials
