...
| Cline | Rules | `.clinerules/ideamart.md` | Copy into your project's `.clinerules/` |
| Kiro | Rules | `.kiro/steering/ideamart.md` | Copy to `~/.kiro/steering/` or the project's `.kiro/steering/` |
...
├── .cursor/rules/ideamart.mdc     ← generated (with .mdc frontmatter)
├── .windsurf/rules/ideamart.md    ← generated
├── .clinerules/ideamart.md        ← generated
├── .kiro/steering/ideamart.md     ← generated
├── .qoder/rules/ideamart.md       ← generated
├── .agents/rules/ideamart.md      ← generated
...
The generated copies come from `scripts/sync-rules.mjs`. Edit `AGENTS.md` and rerun it; CI
fails if they drift.

## No plugin support? You lose nothing important

The tiers differ in *how the instructions load*, not in what the skill knows. A rules-tier
agent still has the complete `references/`, and can still run:
